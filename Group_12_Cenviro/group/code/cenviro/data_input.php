<?php
/*This page allows users to input ESG data.*/
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $period_date = $_POST['period_date'] ?? date('Y-m-d');
    
    if ($category) {
        $saved = false;
        
        // Map form fields to KPI patterns
        if ($category === 'environmental') {
            ...
            $kpi_map = [
                ...
                'kpi_employee_turnover' => 'Employee Turnover'
            ];
        ...
        // Save each field that has a value
        if (isset($kpi_map)) {
            foreach ($kpi_map as $field => $kpi_pattern) {
                if (isset($_POST[$field]) && $_POST[$field] !== '') {
                    $value = floatval($_POST[$field]);
                    $kpi_id = $esgData->getKpiId($kpi_pattern, $category);
                    
                    if ($kpi_id) {
                        // Use ESGData class to save the data
                        if ($esgData->saveData($user_id, $kpi_id, $category, $value, $period_date)) {
                        $saved = true;
                        }
                    }
                }
...
<body>
                ...
                <div id="social" class="tab-content">
                    ...
                    <form method="POST" action="data_input.php">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                <label for="kpi_employee_turnover">Employee Turnover</label>
                                <input type="number" id="kpi_employee_turnover" name="kpi_employee_turnover" placeholder="e.g., 5">
                                <p class="help-text">Number of employees who left during the period</p>
                            </div>
                        </div>
    ...
    <script>
        ...
        function validateForm(form) {
            const periodDate = form.querySelector('input[name="period_date"]');
            ...
            if (!periodDate || !periodDate.value) {
                ...
                if (periodDate) periodDate.style.borderColor = '#dc3545';
            } else {
                if (periodDate) periodDate.style.borderColor = '#ddd';
            }
            ...
            inputs.forEach(input => {
                if (input.value && input.value.trim() !== '') {
                    ...
                    if (isNaN(value) || value < 0) {
                        errors.push(input.previousElementSibling.textContent + ' must be a valid positive number');
            ...
            if (!hasData) {
                errors.push('Please enter at least one data value');
            }
